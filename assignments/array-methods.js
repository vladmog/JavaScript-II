...
const total = runners.reduce(function(sum, r){
  return sum + r.donation
}, 0)
...
// Problem 1
//Director needs an array of names along with shirt sizes to send shirt maker without other information. Use forEach to make array

let nameAndShirt = [];

runners.forEach(runners => {
  nameAndShirt.push(`${runners.first_name} ${runners.shirt_size}`);
})

console.log(nameAndShirt);

// Problem 2
//Director needs list of runners whose names are three or less letters long. 

let shortNames = [];

shortNames = runners.filter((runners) => {
    return runners.first_name.length <= 3;
})

console.log(shortNames);

// Problem 3
//Director needs to disqualify all the freeloaders, make an array of runners who donated less than $20

let freeLoaders = [];

freeLoaders = runners.filter((runners) => {
    return runners.donation < 20;
})

console.log(freeLoaders);